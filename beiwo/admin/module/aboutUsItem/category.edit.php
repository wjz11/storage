...
if (!empty($_GET['id'])) {
    $row = $db->get('tb_about_us_category', '*', ['tbid' => $_GET['id']]);
}
?>
<!DOCTYPE html>
<html>
                ...
                <div class="col-sm-12">
                    <form class="form-horizontal" id="editForm" action="ajax.php">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-10">
                                <p class="form-control-static lookimg">
                                    <?php echo $row['image'] ? '<img class="img-thumbnail" style="max-width:490px;max-height:360px;" src="' . SITEROOTURL . $row['image'] . '"  />' : '【未上传图片】' ?>
                                </p>
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-10">
                                <textarea type="text" name="content" id="content" placeholder="请输入内容"
                                          class="form-control" style="min-height: 100px" dataType="*" nullmsg="请输入内容"
                                          errormsg="请输入内容"><?php echo $row['content']; ?></textarea>
                            </div>
                        </div>
                        <div class="form-group">
                            <label class="col-sm-2 control-label">排序</label>
                            <div class="col-sm-10">
                                <input type="text" name="indexid" id="indexid" placeholder="请输入排序" class="form-control"
                                       value="<?php echo $row['indexid']; ?>" dataType="n" nullmsg="请输入排序"
                                       errormsg="请输入排序">
                                <span class="help-block m-b-none"></span>
                            </div>
                        </div>
...
<script>
    $(function () {
        ...
        uploader.on('uploadSuccess', function (file, cb) {
            if (cb.state == 'SUCCESS') {
                $('#upload-group').prev('p').html('<img class="img-thumbnail" style="max-width:490px;max-height:360px;" src=" ' + cb.preview + '"/>');
                $('#image').val(cb.url);
            } else {
                alert(cb.state);
            }
