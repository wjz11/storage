...
require '../../../global.php';
//进入页面后先判断是否有访问权限
checkAdminAuthorizeAndJump('banner.image_edit');
if (isset($_GET['id']))
{
    $banner = $db->get('tb_banner', '*', array('tbid' => $_GET['id']));
}
if ($_GET['banner_category_id'])
{
    $category_info = $db->get('tb_banner_category', '*', ['tbid' => $_GET['banner_category_id']]);
    $banner['c_width'] = $category_info['width']?$category_info['width']:1920;
    $banner['c_height'] = $category_info['height']?$category_info['height']:820;
}
?>
<!DOCTYPE html>
<html>
            ...
            <div class="ibox float-e-margins">
                <div class="ibox-title">
                    <h2>
                        ...
                        <?php
                        if ($_GET['id'] || ($_GET['banner_category_id'] && $_GET['banner_category_id'] != 1))
                        {
                            ?>
                            编辑Banner
                            <?php
                ...
                <div class="ibox-content">
                    <div class="row row-lg">
                        <div class="col-sm-12">
                            <form class="form-horizontal" id="editForm" action="banner.ajax.php">
                                <input type="hidden" name="ac" value="edit">
                                <input type="hidden" name="id" value="<?php echo $banner['tbid']; ?>">
                                <input type="hidden" name="banner_category_id" value="<?php echo $_GET['banner_category_id']; ?>">
                                <div class="form-group">
                                    <label class="col-sm-2 control-label">标题</label>
                                    <div class="col-sm-10">
                                        <input type="text" name="name" id="name" placeholder="请输入标题" class="form-control" value="<?php echo $banner['name']; ?>" dataType="*" nullmsg="请输入标题">
                                ...
                                <div class="form-group"  id="typediv">
                                    ...
                                    <div class="col-sm-10">
                                        <select class="form-control" id="type" name="type" dataType="*" nullmsg="请选择类型">
                                            ...
                                            <option value="<?php echo 3; ?>" <?php
                                            if ($banner['type'] == 3)
                                            {
                                                ?>selected<?php } ?>>商品</option>
                                            <option value="<?php echo 8; ?>" <?php
                                            if ($banner['type'] == 8)
                                            {
                                                ?>selected<?php } ?>>新闻咨询</option>
                                            <option value="<?php echo 9; ?>" <?php
                                            if ($banner['type'] == 9)
                                            {
                                                ?>selected<?php } ?>>行业咨询</option>
                                        </select>
                                        <span class="help-block m-b-none"></span>
                                    </div>
                                </div>
                                ...
                                if($banner['type']==1){
                                    ...
                                    <div class="col-sm-10">
                                        <input type="text" name="resource_id" id="resource_id" placeholder="请输入资源id" class="form-control" value="<?php echo $banner['resource_id']; ?>" >
                                        <span class="help-block m-b-none"></span>
                                    </div>
                                </div>
                                <div class="form-group lj" 
                                <?php 
                                if($banner['type']!=1){
                                    echo 'style="display:none"';
                                }?>>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-10">
                                        <p class="form-control-static lookimg">
                                            <?php echo $banner['image'] ? '<img class="img-thumbnail"  style="max-width:480px;max-height:205px;" src="' . SITEROOTURL . $banner['image'] . '"/>' : '【未上传图片】'; ?>
                                        </p>
                                        ...
                                        <div class="alert alert-warning">图片大小不超过2MB，图片尺寸最小比例<span
                                                class="imagewh"> <?php echo $banner['c_width'] ?> x <?php echo $banner['c_height'] ?></span></div>
                                    </div>
                                    <span class="help-block m-b-none"></span>
                                </div>
                                <div class="form-group">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-sm-10" style="padding-top:7px;">
                                        <div class="onoffswitch">
                                            <input id="is_use" name="is_use" class="onoffswitch-checkbox"
                                                   type="checkbox" <?php
                                                   if ($banner['is_use'])
                                                   {
                                                       ?> checked="" <?php } ?>>
                                            <label class="onoffswitch-label" for="is_use">
        ...
        <script>
            $(function () {
                ...
                uploader1 = WebUploader.create({
                    ...
                    // 文件接收服务端。
                    server: 'banner.ajax.php?ac=upload1&w=<?php echo $banner["c_width"];?>&h=<?php echo $banner["c_height"];?>',
                    // 选择文件的按钮。可选。
