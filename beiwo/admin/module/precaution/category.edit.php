...
if (!empty($_GET['id'])) {
    $row = $db->get('tb_precaution_category', '*', ['tbid' => $_GET['id']]);
}
?>
<!DOCTYPE html>
<html>
                ...
                <div class="col-sm-12">
                    <form class="form-horizontal" id="editForm" action="ajax.php">
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-10">
                                <p class="form-control-static lookimg"><?php echo $row['image'] ? '<img class="img-thumbnail" style="max-width:680px;max-height:328px;" src="' . SITEROOTURL . $row['image'] . '"  />' : '【未上传图片】' ?></p>
                                ...
                                <div class="alert alert-warning">图片大小不超过2MB，图片尺寸最小比例<span
                                            class="imagewh"> 680 x 328</span></div>
                                <span class="help-block m-b-none"></span>
                            </div>
                        </div>
...
<script>
    $(function () {
        ...
        uploader.on('uploadSuccess', function (file, cb) {
            if (cb.state == 'SUCCESS') {
                $('#upload-group').prev('p').html('<img class="img-thumbnail" style="max-width:680px;max-height:328px;" src="' + cb.preview + '"  />');
                $('#image').val(cb.url);
            } else {
                alert(cb.state);
            }
